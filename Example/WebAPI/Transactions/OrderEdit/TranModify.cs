...
namespace WebAPI.Transactions.OrderEdit
{
  public partial class OrderEditTransaction : TransactionBase
  {
    ...
    public bool Modify(ModifyRequest request)
    {
      var result = false;
      ...
      try
      {
        ...
        var ordereditResult = ordereditRepository.Modify(saveData);
      ...
      {
        // 一意制約違反などの場合はログ出力
        logger.LogError($"{nameof(OrderEditTransaction)}.{nameof(Modify)}:\"{ex.Message}\"");
